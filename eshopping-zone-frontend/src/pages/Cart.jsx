...
export default function Cart() {
    ...
    const handleQuantityChange = async (cartId, itemId, newQuantity) => {
        if (newQuantity <= 0) {
            ...
            await handleUpdateQuantity(cartId, itemId, newQuantity);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-black text-white p-6 md:px-20 mt-16 transition-all duration-500 ease-in-out">
                    ...
                    <div className="md:col-span-2 space-y-6">
                        <AnimatePresence>
                            {cart.items.map((item) => (
                                ...
                                <div className="flex items-center space-x-2">
                                    <button
                                        onClick={() => handleQuantityChange(cart.cartId, item.itemId, item.quantity - 1)}
            ...
            <LoadingScreen show={loading} message="Placing your order" />
            <LoginPromptModal show={showLoginModal}/>
        </div>
    );
}
