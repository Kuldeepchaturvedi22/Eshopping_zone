...
// import { fetchAllOrders, changeOrderStatus as changeOrderStatusApi } from "../api/order";
...
//                 <h2 className="text-2xl font-bold">All Orders (Admin View)</h2>
...
//                         return (
//                             <li key={id} className="border rounded-lg p-4 bg-white shadow-sm hover:shadow-md transition">
//                                 <div className="flex items-start justify-between">
//                                     <div>
//                                         <div className="font-semibold">Order #{id}</div>
//                                         <div className="text-sm text-gray-500">
//                                             Status:{" "}
//                                             <span className="px-2 py-0.5 text-xs rounded bg-gray-100 text-gray-700">{status}</span>
//                                         </div>
//                                         <div className="text-sm text-gray-600 mt-1">Amount: ₹{String(amount)}</div>
//                                     </div>
...
import React, {useCallback, useEffect, useMemo, useState} from "react";
import {changeOrderStatus as changeOrderStatusApi, fetchAllOrders,} from "../api/order";
import {getAuthToken} from "../api/_auth";
...
function AdminOrders() {
    ...
    const handleChangeStatus = useCallback(async (orderId, newStatus) => {
        if (!newStatus) return;
        const yes = window.confirm(`Change status of order #${orderId} to ${newStatus}?`);
        if (!yes) return;
        try {
    ...
    return (<div className="max-w-6xl mx-auto p-4 mt-16">
            <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex flex-col md:flex-row gap-3 mb-4">
                <input
                    ...
                    onChange={(e) => setQuery(e.target.value)}
                    placeholder="Search by Order ID or status…"
                    className="px-3 py-2 border rounded w-full md:w-64 dark:bg-gray-800 dark:text-gray-100 dark:border-gray-600"
                />
                <select
                    value={statusFilter}
                    onChange={(e) => setStatusFilter(e.target.value)}
                    className="px-3 py-2 border rounded w-full md:w-48 dark:bg-gray-800 dark:text-gray-100 dark:border-gray-600"
                >
            ...
            {loading ? (<div className="space-y-3">
                    ...
                    className="p-3 bg-red-50 dark:bg-red-900 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-700 rounded">
                    {errorMsg}
                ...
                <ul className="grid md:grid-cols-2 gap-4">
                    {filtered.map((o) => {
                        ...
                        return (<li
                                key={id}
                                className="border rounded-lg p-4 bg-white dark:bg-gray-800 dark:border-gray-700 shadow-sm hover:shadow-md transition"
                            >
                                <div className="flex items-start justify-between">
                                    <div>
                                    ...
                                    <div className="text-right">
                                        <label className="text-sm text-gray-600 dark:text-gray-300">
                                            ...
                                            <select
                                                ...
                                                className="ml-2 border rounded px-2 py-1 text-sm dark:bg-gray-800 dark:text-gray-100 dark:border-gray-600"
                                            >
                                                <option value="">Select</option>
                                                <option value="PENDING">PENDING</option>
                                                <option value="SHIPPED">SHIPPED</option>
