...
public class OrderServiceController {
    ...
    @Operation(summary = "Get detailed orders by customer ID")
    @GetMapping("/customer/{customerId}/details")
    public ResponseEntity<List<OrderDetailDTO>> getDetailedOrdersByCustomerId(@PathVariable int customerId) {
        List<OrderDetailDTO> orders = orderService.getDetailedOrdersByCustomerId(customerId);
        return new ResponseEntity<>(orders, HttpStatus.OK);
    }

    @Operation(summary = "Get detailed order by order ID")
    @GetMapping("/details/{orderId}")
    public ResponseEntity<OrderDetailDTO> getOrderDetails(@PathVariable int orderId) {
...
//    }
    ...
    @Operation(summary = "Get today's orders (DATE-only storage)")
    @GetMapping("/latest-orders")
    public ResponseEntity<List<Order>> getLatestOrderByTime() {
        // Controller still uses LocalDateTime to preserve service signature,
        // but the service will convert to LocalDate and query for that date.
        LocalDateTime startOfToday = LocalDate.now().atStartOfDay();
        List<Order> recentOrders = orderService.getLatestOrdersWithTime(startOfToday);
        return ResponseEntity.ok(recentOrders);
    }
