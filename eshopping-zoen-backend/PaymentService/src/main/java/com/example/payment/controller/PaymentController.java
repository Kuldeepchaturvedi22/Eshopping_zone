...
public class PaymentController {
    ...
    public RedirectView paymentCallback(
        ...
        try {
            ...
            log.info("Updating payment status for order: {} with payment ID: {}", originalOrderId, razorpayPaymentId);
            orderServiceClient.updatePaymentStatus(payment);
            log.info("Payment status updated successfully for order: {}", originalOrderId);

            // Redirect to frontend success page with order details
            String redirectUrl = String.format("http://localhost:5173/payment-success?orderId=%s&amount=%.2f&status=SUCCESS", 
                    originalOrderId, amountInRupees.doubleValue());
            ...
            try {
                ...
                log.info("Updating failed payment status for order: {}", originalOrderId);
                orderServiceClient.updatePaymentStatus(payment);
                log.info("Failed payment status updated for order: {}", originalOrderId);

                // Redirect to frontend error page
                String redirectUrl = String.format("http://localhost:5173/payment-success?orderId=%s&amount=%.2f&status=FAILED", 
                        originalOrderId, amountInRupees.doubleValue());
                return new RedirectView(redirectUrl);
            } catch (Exception ex) {
                log.error("Failed to update order after payment failure", ex);
                return new RedirectView("http://localhost:5173/payment-success?status=ERROR");
            }
        }
    }
